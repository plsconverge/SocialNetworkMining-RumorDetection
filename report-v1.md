...
（本部分由队友负责，展示数据分布图、传播图等可视化结果，分析谣言与非谣言在时空维度的差异。）
...
随机森林是我们选择的第一个基准模型。作为一种经典的集成学习方法，它通过构建多棵决策树并进行投票来做出最终预测。在本项目中，随机森林主要依赖于我们要动挖掘的“手工特征”。

我们设计了一个复杂的特征工程模块，从传播树中提取了数十维特征，包括但不限于：
- **传播特征**：树的深度、广度、总节点数，以及短时间内连续转发的爆发性指标。
- **用户特征**：参与传播用户的平均粉丝数、认证用户比例等。
- **时序特征**：传播发生的时间段分布（如是否发生在深夜）。

**算法原理**：随机森林通过在训练数据上随机采样（Bootstrap）并随机选择特征子集来训练每一棵决策树，最后通过集成投票降低了模型的方差，提高了泛化能力。它不需要复杂的调参即可获得不错的基线效果，且对异常值具有较好的鲁棒性。



### 2. XGBoost (梯度提升树)

在随机森林的基础上，我们进一步尝试了 XGBoost 模型。与随机森林的并行集成不同，XGBoost 采用的是 Boosting 策略，即通过迭代地训练新模型来拟合旧模型的残差（预测误差），从而不断提升整体的预测精度。

**算法原理与实现细节**：
在代码实现中，我们同样使用了上述特征工程提取的数值特征。XGBoost 的核心优势在于其对损失函数的二阶泰勒展开以及正则化项的引入，这使得它在防止过拟合的同时，能够更精细地捕捉特征与标签之间的非线性关系。我们在训练脚本中加入了**特征重要性分析**（Feature Importance Analysis），这不仅帮助我们提升了模型效果，还让我们直观地看到了哪些特征最能区分谣言——例如，我们发现“转发频率”和“用户粉丝数的方差”往往是区分真假消息的关键指标。
...
我们的 BERT 模型实现了一个独特的 **多模态后期融合架构**。社交媒体数据天然包含“非结构化文本”和“结构化数值”两类信息。我们的模型包含两个并行的特征提取流：1）文本流：利用 bert-base-chinese 作为骨干网络，通过 Transformer 编码器提取深层语义特征，取 [CLS] 向量作为整个事件的语义表示。2）数值流：将粉丝数、点赞数等数值特征标准化后，通过一个多层感知机 (MLP) 映射到高维空间。最终，我们将两路特征在分类层前进行拼接融合。这种设计使得模型既能理解“文本说了什么”（语义维度），又能考量“账号可信度如何”（社会学维度）。

### 4. LLM (Large Language Model - Gemini)
